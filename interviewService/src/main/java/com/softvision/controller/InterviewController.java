...
public class InterviewController {
        ...
        if ((candidateId != null && !candidateId.isEmpty())
            ...
            CompletableFuture.supplyAsync(() -> {
                            ...
                            .thenApply(v -> {
                                ...
                                return emailService.sendEmail(buildEmail(context,"published",ServiceConstants.CANDIDATE_PUBLISHED).apply(interviewerList));
                            })
                            .exceptionally( e -> {
    ...
    public void acknowledged(@Suspended AsyncResponse asyncResponse,
        ...
        if ((id != null && !id.isEmpty()) && (interviewerId != null && !interviewerId.isEmpty())) {
            CompletableFuture.supplyAsync(() -> acknowledgedStatus.acknowledgedInterview(id, interviewerId))
                            .thenApply(optional -> {
                                try {
                                    asyncResponse.resume(optional.get());
                                } catch(Exception ex) {
                                    LOGGER.error("Exception while acknowledging the candidate {} ", ex.getMessage());

                                }
                                return optional.get();
                            })
                            .exceptionally(e -> {
                                LOGGER.error("Error while resuming data {}", e.getMessage());
                                asyncResponse.resume(
                                        Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(e.getMessage()).build());
                                return null;
                            })
                            .thenApply(interviewlog -> {
                                System.out.println("Optional Data " + interviewlog);
                                Candidate candidate = serviceClientHelper.getCandidateById(interviewlog.getCandidateId());
                                Optional<List<Employee>> employee = serviceClientHelper.getInterviewer(interviewlog.getInterviewerId());
                            ...
                            .exceptionally(e -> {
                                e.printStackTrace();
                                LOGGER.error("Error while sending email for Acknowledged {} " , e.getMessage());
                                return null;
                            });
    ...
    private Function<Collection<Employee>,Email> buildEmail(Context context,String templateType,String subject) {
        return (e -> {
    ...
    private Function<Map<String,Object>,Map<String,Object>> acknowledgedDataBuilder(Employee employee ) {
        return( m -> {
            Map<String,Object> map = new HashMap<>(m);
