package com.softvision.common;

/**
 * The Class ServiceConstants.
 *
 * @author arun.p
 */
public class ServiceConstants {


    /**
     * The Constant CANDIDATE_SERVICE.
     */
    public static final String CANDIDATE_SERVICE = "candidateService";
    /**
     * The Constant ADMIN
     */
    public static final String ADMIN = "admin";

    /**
     * The Constant INTERVIEWER.
     */
    public static final String INTERVIEWER = "interviewer";

    /**
     * The Constant RECRUITER.
     */
    public static final String RECRUITER = "recruiter";

    /**
     * The constant GETBYBANDEXP
     */
    public static final String GET_BY_BAND_EXP = "getbybandexp";

    /**
     * The constant GETTECH
     */
    public static final String GET_TECH = "gettech";

    /**
     * The constant LOGIN
     */
    public static final String LOGIN = "login";

    /**
     * The Constant BACK_SLASH.
     */
    public static final String BACK_SLASH = "/";

    /**
     * The Constant ADD.
     */
    public static final String ADD = "add";

    /**
     * The Constant UPDATE.
     */
    public static final String UPDATE = "update";

    /**
     * The Constant DELETE.
     */
    public static final String DELETE = "delete";

    /**
     * The Constant CANDIDATES.
     */
    public static final String CANDIDATES = "candidates";

    /**
     * The Constant ALL_CANDIDATES.
     */
    public static final String ALL_CANDIDATES = "allcandidates";

    /**
     * The Constant CANDIDATE.
     */
    public static final String CANDIDATE = "candidate";

    /**
     * The Constant ID.
     */
    public static final String ID = "id";

    /**
     * The Constant OPENING_CURLEY_BRACKET.
     */
    public static final String OPENING_CURLY_BRACKET = "{";

    /**
     * The Constant CLOSING_CURLEY_BRACKET.
     */
    public static final String CLOSING_CURLY_BRACKET = "}";

    /**
     * The Constant ACCEPT_APPLICATION_JSON.
     */
    public static final String ACCEPT_APPLICATION_JSON = "Accept=application/json";

    /**
     * The Constant BLANK_SPACE.
     */
    public static final String BLANK_SPACE = " ";

    /**
     * The Constant SEARCH.
     */
    public static final String SEARCH = "search";

    /**
     * The Constant CANDIDATE_NOT_FOUND.
     */
    public static final String CANDIDATE_NOT_FOUND = "The selected candidate found";

    /**
     * The Constant CANDIDATE_DELETED.
     */
    public static final String CANDIDATE_DELETED = "Candidate has been deleted successfully";

}
